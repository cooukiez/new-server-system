    ...
    users =
      (lib.mapAttrs (
          username: userConfig: {
            ...
            age.secrets.codeberg-token.file = ../../secrets/codeberg-token.age;

            programs.git = let
              # github helper
              githubSecretHelperScript = ''
                if [ "$1" = "get" ]; then
              ...
              githubSecretHelper = pkgs.writeShellScript "github-secret-helper" githubSecretHelperScript;

              # codeberg helper
              codebergSecretHelperScript = ''
                if [ "$1" = "get" ]; then
                  token=$(cat ${config.age.secrets.codeberg-token.path})
                  echo "username=${userConfig.gitName}"
                  echo "password=$token"
                fi
              '';

              codebergSecretHelper = pkgs.writeShellScript "codeberg-secret-helper" codebergSecretHelperScript;
            in {
              enable = true;
              ...
              settings = {
                ...
                "credential \"https://github.com\"" = {
                  helper = "${githubSecretHelper}";
                };

                "credential \"https://codeberg.org\"" = {
                  helper = "${codebergSecretHelper}";
                };
              };
            };
