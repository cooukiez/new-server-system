...
def prefetch_nix_hash(repo, digest):
    ...
    # use the digest specifically to ensure we hash the exact version
    cmd = ["nix-prefetch-docker", repo, "--image-digest", digest]
    result = subprocess.run(cmd, capture_output=True, text=True)
    if result.returncode != 0:
...
def main():
    ...
    with open(NIX_FILE, "w") as f:
        f.write("# this file is generated by fetch-images.py\n{\n")
    ...
    print(f"\nFinished. Updated {LOCK_FILE} and {NIX_FILE}")
